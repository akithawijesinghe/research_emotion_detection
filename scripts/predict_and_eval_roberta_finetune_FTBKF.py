...
# ==================================================
# ✅ PUT THE *REAL* CHECKPOINT DIRECTORY HERE
# ==================================================
MODEL_DIR = "results/roberta_finetune/checkpoint-3822"
OUTPUT_PREFIX = "roberta_finetuned"
...
# -------------------------
print("🔹 Loading RoBERTa fine-tuned model...")
...
for text in test_df["comment"]:
    outputs = pipe(text)[0]   # list of dicts
...
prob_df.to_csv(f"results/{OUTPUT_PREFIX}_test_predictions.csv", index=False)
...
plt.ylabel("True")
plt.tight_layout()
plt.savefig(f"results/{OUTPUT_PREFIX}_confusion_matrix.png")
plt.close()
...
mis_df.to_csv(f"results/{OUTPUT_PREFIX}_misclassified.csv", index=False)
print("✅ Saved misclassified samples")

print("✅ RoBERTa fine-tuned evaluation complete!")
