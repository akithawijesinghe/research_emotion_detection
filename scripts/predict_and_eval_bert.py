...
# # scripts/predict_and_eval_bert.py = for getting predictions from BERT model and saving evaluation metrics
# import json
# import pandas as pd
# from transformers import AutoTokenizer, AutoModelForSequenceClassification, pipeline
# from datasets import load_from_disk
...
#     best = max(outputs, key=lambda x: x['score'])
#     pred_names.append(best['label'])
#     pred_ids.append(reverse_map[best['label']])
    
#     row = [0.0] * len(label_names)
#     for o in outputs:
#         row[reverse_map[o['label']]] = o['score']
#     prob_rows.append(row)

# # === Save prediction + probability file for ensemble ===
# pred_df = pd.DataFrame(prob_rows, columns=label_names)
# pred_df["true_label_id"] = labels
...
# print(f"✅ Saved: {MODEL_NAME}_classification_report.csv")
...
# mis = df[df["true_label"] != df["pred_label"]]
# mis.sample(min(100, len(mis)), random_state=42).to_csv(f"results/{MODEL_NAME}_misclassified_sample_100.csv", index=False)
# print(f"✅ Saved: {MODEL_NAME}_misclassified_sample_100.csv")
